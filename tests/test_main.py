...
from sysdiag_analyzer.datatypes import (
    SystemReport,
    UnitHealthInfo,  # Add UnitHealthInfo
)
...
def mock_unit_list():
    ...
    return [
        UnitHealthInfo(name="unit1.service", path="/fake/path1"),
        UnitHealthInfo(name="unit2.target"),
    ]
...
def test_run_command_flags(
    ...
    mock_get_units_json.return_value = (
        [],
        "Fallback not needed",
    )  # Ensure fallback isn't used here
    ...
    mock_run_full.assert_called_with(
        ...
        dbus_manager=ANY,
        app_config=mock_default_config,
        since=None,
        enable_ebpf=False,
        analyze_full_graph=False,
        analyze_ml=False,
        analyze_llm=False,
        llm_config=None,
    )
    ...
    mock_load_config.return_value = {**mock_default_config, "llm": llm_config}
    with patch("sysdiag_analyzer.main.HAS_LLM_ENGINE", True), patch(
        "sysdiag_analyzer.main.HAS_LLM_OLLAMA", True
    ):
    ...
    mock_run_full.assert_called_with(
        ...
        all_units=mock_unit_list,
        dbus_manager=ANY,
        app_config=mock_load_config.return_value,
        since=None,
        enable_ebpf=False,
        analyze_full_graph=False,
        analyze_ml=False,
        analyze_llm=True,
        llm_config=llm_config,  # Check LLM args passed
    )
...
def test_run_llm_fail_fast_no_provider(
    mock_load_config, mock_run_full, runner, mock_default_config
):
...
@patch("sysdiag_analyzer.main.run_full_analysis")
@patch("sysdiag_analyzer.main.load_config")
def test_run_llm_fail_fast_no_model(
    mock_load_config, mock_run_full, runner, mock_default_config
):
...
def test_save_report_success(temp_dir_path):
    ...
    with patch(
        "sysdiag_analyzer.main.get_boot_id", return_value="test-boot-id"
    ), patch("sysdiag_analyzer.main.datetime") as mock_datetime, patch(
        "sysdiag_analyzer.main.gzip.open", mock_open()
    ) as mock_gz_open, patch(
        "sysdiag_analyzer.main.json.dumps", return_value='{"report": "data"}'
    ) as mock_json_dumps:
...
def test_run_command_integration_save(
    ...
    mock_save_report.assert_called_once_with(
        mock_report, Path(mock_default_config["history"]["directory"])
    )
    mock_apply_retention.assert_called_once_with(
        Path(mock_default_config["history"]["directory"]),
        mock_default_config["history"]["max_files"],
    )
...
def mock_history_dir_show(tmp_path):
    ...
    files_to_create = {
        "report-boot1-20250102T100000Z.jsonl.gz": datetime.datetime(
            2025, 1, 2, 10, 0, 0, tzinfo=datetime.timezone.utc
        ),  # Newest
        "report-boot1-20250102T090000Z.jsonl.gz": datetime.datetime(
            2025, 1, 2, 9, 0, 0, tzinfo=datetime.timezone.utc
        ),  # Second newest
        "report-boot0-20250101T120000Z.jsonl.gz": datetime.datetime(
            2025, 1, 1, 12, 0, 0, tzinfo=datetime.timezone.utc
        ),  # Third newest
        "report-oldest-to-be-excluded.jsonl.gz": datetime.datetime(
...
pytestmark_ml = pytest.mark.skipif(
    not HAS_ML_LIBS_FOR_TEST, reason="Requires ML libraries"
)
...
@patch("sysdiag_analyzer.main.load_config")
@patch("sysdiag_analyzer.main.os.cpu_count", return_value=2)
def test_retrain_ml_success(
    mock_cpu_count,
    mock_load_config,
    mock_check_priv,
    mock_train,
    ...
    mock_train.return_value = (1, {"Zero variance": ["unitB.service"]})
    ...
    mock_load_config.assert_called_once_with(config_path_override=None)
    mock_load_prep.assert_called_once_with(
        history_dir=history_path, num_reports=30, include_devices=False
    )
    mock_engineer.assert_called_once_with(mock_df)
    mock_train.assert_called_once_with(
        engineered_df=mock_df, model_dir_path=model_path, max_workers=2
    )
    assert "Successfully trained and saved 1 anomaly models" in result.stdout
    assert "Skipped a total of 1 units" in result.stdout
    assert "Zero variance" in result.stdout
...
try:
    ...
    try:
        from ..modules.health import _dbus_to_python, _get_unit_properties_dbus, _get_unit_properties_fallback
    except ImportError:
        def _dbus_to_python(val): return val
        def _get_unit_properties_dbus(bus, path): return None
        def _get_unit_properties_fallback(name): return None
    HAS_DBUS = True
except ImportError:
    HAS_DBUS = False
    dbus = None
    def _dbus_to_python(val): return val # Dummy function
    def _get_unit_properties_dbus(bus, path): return None
    def _get_unit_properties_fallback(name): return None
...
from ..datatypes import (
    ...
    SystemResourceUsage,
    UnitResourceUsage,
    ChildProcessGroupUsage, # Added Phase 10
    UnitHealthInfo, # Needed for input type hint
)
from ..utils import run_subprocess # Potentially needed for fallback
...
CHILD_PROCESS_CPU_INTERVAL = 0.1
CHILD_PROCESS_MAX_GROUPS = 50
CHILD_PROCESS_MIN_MEM_MB = 5
CHILD_PROCESS_MIN_CPU_PCT = 1.0
        ...
        for line_num, line in enumerate(content.splitlines()):
            ...
            if len(parts) == 2:
                ...
                try:
                    ...
                    log_cgroup.warning(f"Non-integer value found for key '{key}' in kv content: {parts[1]}")
            elif line.strip():
                 log_cgroup.warning(f"Skipping line with unexpected format in kv content (line {line_num+1}): {line!r}")
    except Exception as e:
...
def _parse_cgroup_cpu_stat(content: Optional[str]) -> Optional[int]:
    ...
    try:
        for line in content.splitlines():
            ...
            if len(parts) == 2:
                ...
                try:
                    ...
                    log_cgroup.warning(f"Non-integer value for key '{key}' in cpu.stat: {parts[1]}")
            elif line.strip():
        ...
        if usage_usec is not None:
            ...
            log_cgroup.warning(f"Could not find 'usage_usec' in parsed cpu.stat data: {data}")
            return None
    except Exception as e:
...
def _parse_cgroup_memory(content: Optional[str]) -> Optional[int]:
    ...
    try:
        ...
        log_cgroup.warning(f"Could not parse integer from memory cgroup content: {content[:100]}")
        return None
...
def _parse_cgroup_io_stat(content: Optional[str]) -> Tuple[Optional[int], Optional[int]]:
    ...
    log_cgroup.debug(f"--- Parsing io.stat content ---")
    ...
    try:
        for line_num, line in enumerate(content.splitlines()):
            ...
            if IO_STAT_DEVICE_LINE_PATTERN.match(line_strip):
                 log_cgroup.debug(f"Line {line_num+1}: Skipping device-specific line: '{line_strip}'")
                 continue
            log_cgroup.debug(f"Line {line_num+1}: Processing aggregate line: '{line_strip}'")
            for part_num, part in enumerate(line_strip.split()):
                log_cgroup.debug(f"  Part {part_num+1}: '{part}'")
                if '=' in part:
                    try:
                        key, value_str = part.split('=', 1)
                        log_cgroup.debug(f"    Found '=': key='{key}', value_str='{value_str}'")
                        if key in ["rbytes", "wbytes"]:
                            log_cgroup.debug(f"      Key matches '{key}'. Checking if value '{value_str}' is digit...")
                            if value_str.isdigit():
                                value = int(value_str)
                                log_cgroup.debug(f"        Value is digit. Parsed value: {value}")
                                if key == "rbytes":
                                    rbytes_sum += value
                                    log_cgroup.debug(f"        Added to rbytes_sum. New sum: {rbytes_sum}")
                                elif key == "wbytes":
                                    wbytes_sum += value
                                    log_cgroup.debug(f"        Added to wbytes_sum. New sum: {wbytes_sum}")
                                found_any_valid_key = True
                            else:
                                log_cgroup.warning(f"      Value '{value_str}' is not purely digits for key '{key}'. Skipping.")
                    except ValueError as e:
                        log_cgroup.warning(f"    ValueError processing part '{part}': {e}")
                    except Exception as e_inner:
                         log_cgroup.warning(f"    Unexpected error processing part '{part}': {e_inner}", exc_info=True)
    except Exception as e:
        log_cgroup.exception(f"Error during main loop of parsing io.stat content: {e}")
        return None, None
    log_cgroup.debug(f"--- Finished parsing io.stat ---")
    ...
    if found_any_valid_key:
        ...
        log_cgroup.debug("No rbytes/wbytes keys found or parsed in aggregate io.stat lines (returning 0, 0).")
        return 0, 0
    else:
...
def _get_unit_cgroup_path(unit_name: str, dbus_manager: Optional[Any]) -> Optional[str]:
    """Gets the relative cgroup path for a unit using DBus (preferred)."""
    # (Logic remains the same)
    log_dbus = log_dbus_res
    log_dbus.debug(f"--- Starting cgroup path lookup for unit: {unit_name} ---")
    if not HAS_DBUS:
        log_dbus.debug("DBus bindings not available (HAS_DBUS=False).")
        return None
    if not dbus:
        log_dbus.error("DBus bindings reported available, but dbus module is None.")
        return None
    if not dbus_manager:
        log_dbus.debug("DBus manager object not provided.")
        return None
    if not hasattr(dbus_manager, 'bus'):
         log_dbus.warning(f"DBus manager object for {unit_name} lacks expected 'bus' attribute.")
         return None
    ...
    cgroup_path_prop: Optional[Any] = None
    properties_interface: Optional[Any] = None
    try:
        ...
        log_dbus.debug(f"Getting object for path: {unit_path}")
        unit_object = dbus_bus.get_object('org.freedesktop.systemd1', unit_path)
        log_dbus.debug(f"Getting DBus.Properties interface for object: {unit_object}")
        properties_interface = dbus.Interface(unit_object, 'org.freedesktop.DBus.Properties')
        log_dbus.debug(f"Properties interface obtained: {properties_interface}")
        interfaces_to_check = [
        ...
        for iface_name in interfaces_to_check:
            try:
                log_dbus.debug(f"Attempting properties_interface.Get('{iface_name}', 'ControlGroup')...")
                cgroup_path_prop = properties_interface.Get(iface_name, 'ControlGroup')
                log_dbus.debug(f"Successfully got ControlGroup property from interface '{iface_name}': {cgroup_path_prop!r}")
                found_prop = True
                break
            except dbus.exceptions.DBusException as get_prop_e:
                dbus_error_name_get = getattr(get_prop_e, '_dbus_error_name', 'UnknownDBusError')
                if 'org.freedesktop.DBus.Error.UnknownMethod' in dbus_error_name_get or \
                   'org.freedesktop.DBus.Error.UnknownInterface' in dbus_error_name_get or \
                   'org.freedesktop.DBus.Error.UnknownProperty' in dbus_error_name_get:
                    log_dbus.debug(f"Property 'ControlGroup' not found on interface '{iface_name}' for {unit_name}. Trying next.")
                else:
                    log_dbus.warning(f"Unexpected DBus error getting ControlGroup from '{iface_name}' for {unit_name}: {get_prop_e}")
        if not found_prop or cgroup_path_prop is None:
            log_dbus.debug(f"Could not find 'ControlGroup' property on any standard interface for unit {unit_name} (expected for some unit types).")
            return None
        relative_path = str(cgroup_path_prop).lstrip('/')
        if not relative_path:
             log_dbus.debug(f"ControlGroup property for {unit_name} resulted in empty relative path ('{cgroup_path_prop}'). Treating as no path.")
             return None
        ...
        log_dbus.warning(f"DBusException occurred during cgroup path lookup for unit '{unit_name}': {e} (Error Name: {dbus_error_name})")
        if unit_path is None:
             log_dbus.warning(f"Failure likely occurred during manager.GetUnit('{unit_name}').")
        elif properties_interface is None:
             log_dbus.warning(f"Failure likely occurred getting Properties interface for path '{unit_path}'.")
        return None
    except Exception as e:
        log_dbus.error(f"Unexpected error getting cgroup path for {unit_name} via DBus: {e}", exc_info=True)
        return None
    finally:
        log_dbus.debug(f"--- Finished cgroup path lookup for unit: {unit_name} ---")

# --- Phase 10: Child Process Monitoring Helpers ---
def _get_service_pids(units: List[UnitHealthInfo], dbus_manager: Optional[Any]) -> Dict[str, Optional[int]]:
    ...
    pids_found_count = 0
    log_child_proc.debug(f"Attempting to fetch PIDs for active services from {len(units)} provided units...")
    for unit in units:
        ...
        if details is None or 'MainPID' not in details:
            log_child_proc.debug(f"Details missing or lack MainPID for active service {unit_name}, attempting fetch...")
            ...
            if isinstance(fetched_details, dict):
                 ...
                 log_child_proc.debug(f"Fetched details for {unit_name} to find MainPID.")
            else:
                 log_child_proc.warning(f"Could not fetch properties for active service {unit_name} to get MainPID.")
                 unit.details = {} # Ensure details is a dict even on failure
            ...
            if pid_val is not None:
                try:
                    ...
                    if pid_int > 0:
                        ...
                        log_child_proc.debug(f"Ignoring MainPID {pid_int} for service {unit_name}")
                except (ValueError, TypeError):
                    log_child_proc.warning(f"Could not parse MainPID '{pid_val}' for service {unit_name}")
        service_pids[unit_name] = pid
        if pid:
             log_child_proc.debug(f"Stored MainPID {pid} for active service {unit_name}.")
    log_child_proc.debug(f"Checked {active_services_count} active services, found {pids_found_count} usable MainPIDs.")
    return service_pids
...
def _scan_and_group_child_processes(service_pids: Dict[str, Optional[int]]) -> List[ChildProcessGroupUsage]:
    """Scans process tree, groups by command, aggregates resources."""
    # (Logic remains the same)
    log_child_proc.info("Scanning for child process groups...")
    groups: Dict[Tuple[str, str], ChildProcessGroupUsage] = {} # (parent_unit, cmd_name) -> GroupData
    processed_pids: Set[int] = set() # Avoid processing the same PID multiple times
    try:
        psutil.cpu_times_percent(interval=0.01, percpu=False)
    except Exception as e:
        log_child_proc.warning(f"Could not get initial CPU times for percentage calculation: {e}. CPU % will be instantaneous.")
    for unit_name, main_pid in service_pids.items():
        ...
        try:
            ...
            log_child_proc.debug(f"Found {len(children)} recursive children for {unit_name} (PID: {main_pid})")
            for child in children:
                ...
                try:
                    ...
                    with child.oneshot(): # Use oneshot for efficiency
                        ...
                        # Use interval=None for instantaneous CPU% after initial call
                        cpu_percent = child.cpu_percent(interval=None)
                    if not cmdline:
                        log_child_proc.debug(f"Skipping child PID {child.pid} (parent: {main_pid}) - empty cmdline")
                        continue
                    ...
                    if group.aggregated_cpu_percent is not None and cpu_percent is not None:
                         group.aggregated_cpu_percent += cpu_percent
                    if group.aggregated_memory_bytes is not None and mem_info:
                         group.aggregated_memory_bytes += mem_info.rss
                except psutil.NoSuchProcess:
                    log_child_proc.debug(f"Child process {child.pid} disappeared during analysis.")
                except psutil.AccessDenied:
                    log_child_proc.warning(f"Access denied getting info for child process {child.pid} of {unit_name}")
                except Exception as child_e:
                    log_child_proc.error(f"Error processing child {child.pid} of {unit_name}: {child_e}", exc_info=False) # Avoid excessive logging
        except psutil.NoSuchProcess:
            log_child_proc.debug(f"Main service process {main_pid} for {unit_name} disappeared during child scan.")
        except psutil.AccessDenied:
             log_child_proc.warning(f"Access denied accessing main service process {main_pid} for {unit_name}")
        except Exception as parent_e:
            log_child_proc.error(f"Error processing children of {unit_name} (PID: {main_pid}): {parent_e}", exc_info=False)
    # Filter groups based on thresholds
    filtered_groups = []
    for group in groups.values():
        mem_mb = (group.aggregated_memory_bytes or 0) / (1024 * 1024)
        cpu_pct = group.aggregated_cpu_percent or 0.0
        if group.process_count > 0 and (mem_mb >= CHILD_PROCESS_MIN_MEM_MB or cpu_pct >= CHILD_PROCESS_MIN_CPU_PCT):
            filtered_groups.append(group)
        else:
             log_child_proc.debug(f"Filtering out group '{group.command_name}' under parent '{group.parent_unit}' (Count: {group.process_count}, CPU: {cpu_pct:.1f}%, Mem: {mem_mb:.1f}MB)")
    # Sort and limit results
    filtered_groups.sort(key=lambda g: (g.aggregated_cpu_percent or 0.0, g.aggregated_memory_bytes or 0), reverse=True)
    final_groups = filtered_groups[:CHILD_PROCESS_MAX_GROUPS]
    log_child_proc.info(f"Child process scan complete. Found {len(groups)} initial groups, reporting {len(final_groups)} groups after filtering.")
    return final_groups
    ...
    try:
        ...
        try:
            ...
            if disk_io:
             ...
             usage.error = f"{usage.error}; {disk_err_str}" if usage.error else disk_err_str
        try:
...
def get_unit_resource_usage(units: List[UnitHealthInfo], dbus_manager: Optional[Any]) -> List[UnitResourceUsage]:
    ...
    if not CGROUP_BASE_PATH.is_dir():
        log.error(f"Cgroup base path {CGROUP_BASE_PATH} not found. Cannot collect per-unit cgroup stats.")
        return results
    for unit_info in units:
        ...
        if not relative_cgroup_path:
            log_cgroup.debug(f"Skipping cgroup resource collection for {unit_name}: No cgroup path found.")
        ...
        if not full_cgroup_path.is_dir():
             log_cgroup.warning(f"Specific cgroup directory not found: {full_cgroup_path}. Skipping resource collection for {unit_name}.")
             results.append(usage)
             continue
        ...
        log_cgroup.debug(f"Calling _parse_cgroup_io_stat for {unit_name} with content (snippet): {io_stat_content[:100] if io_stat_content else 'None'}...")
        rbytes, wbytes = _parse_cgroup_io_stat(io_stat_content)
        log_cgroup.debug(f"Parser _parse_cgroup_io_stat for {unit_name} returned: rbytes={rbytes}, wbytes={wbytes}")
        ...
        if error_parts:
            ...
            log.warning(f"Partial errors fetching resources for {unit_name}: {usage.error}")
        log_cgroup.debug(
        ...
        results.append(usage)
    log.debug(f"Finished collecting cgroup resource usage for {len(results)} units.")
    return results
...
def analyze_resources(
    ...
    try:
        ...
        if result.system_usage and result.system_usage.error:
            ...
            result.analysis_error = f"{result.analysis_error}; {sys_err}" if result.analysis_error else sys_err
    except Exception as e:
        log.exception("Unexpected error getting system-wide usage.")
        err_msg = f"Failed to get system usage: {e}"
        result.analysis_error = f"{result.analysis_error}; {err_msg}" if result.analysis_error else err_msg
    ...
    if not units:
         ...
         log.warning("DBus bindings not installed, cannot perform per-unit resource analysis via cgroups.")
         err_msg = "DBus bindings not installed"
         result.analysis_error = f"{result.analysis_error}; {err_msg}" if result.analysis_error else err_msg
         result.unit_usage = []
    elif not dbus_manager:
         # Check if dbus is installed but manager connection failed earlier
         log.warning("DBus manager connection unavailable, cannot perform per-unit resource analysis via cgroups.")
    ...
    if result.unit_usage:
        try:
            ...
            valid_mem_units = [u for u in result.unit_usage if u.memory_current_bytes is not None]
            valid_io_units = [u for u in result.unit_usage if u.io_read_bytes is not None and u.io_write_bytes is not None and (u.io_read_bytes > 0 or u.io_write_bytes > 0)]
            result.top_cpu_units = sorted(valid_cpu_units, key=lambda u: u.cpu_usage_nsec or 0, reverse=True)[:TOP_N_COUNT]
            result.top_memory_units = sorted(valid_mem_units, key=lambda u: u.memory_current_bytes or 0, reverse=True)[:TOP_N_COUNT]
            result.top_io_units = sorted(valid_io_units, key=lambda u: (u.io_read_bytes or 0) + (u.io_write_bytes or 0), reverse=True)[:TOP_N_COUNT]
            log.debug(f"Identified Top {TOP_N_COUNT} unit consumers: CPU={len(result.top_cpu_units)}, Mem={len(result.top_memory_units)}, IO={len(result.top_io_units)}")
        except Exception as e:
        ...
        result.top_io_units = []
        log.debug("Skipping Top N unit calculation as per-unit usage data is unavailable.")
